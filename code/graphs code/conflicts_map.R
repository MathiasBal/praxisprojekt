...
nigeria.top.conflicts <- nigeria.merged %>%
  mutate(actor_group = ifelse(actor_group1 < actor_group2, 
                              paste(actor_group1, "vs", actor_group2), 
                              paste(actor_group2, "vs", actor_group1))) %>%
  filter(actor_group %in% top.conflicts) %>% 
...
ggplot() +
  ...
  geom_point(data = nigeria.top.conflicts, 
             aes(x = longitude, y = latitude, 
                 color = actor_group_de), size = 1.3) + 
  geom_point(data = big.cities, aes(x = lon, y = lat), 
             color = "black", size = 5) + 
  geom_label_repel(data = big.cities, aes(x = lon, y = lat, label = city),
